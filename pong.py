...
class QLearningAlgorithm():
    def __init__(self, actions, discount, featureExtractor, explorationProb, rand):
    ...
    def getAction(self, state, done):
        if done:
            return 0
        #random choice
        if random.random() < self.explorationProb/self.rand:
...
def featureExtractorState(state, action):
    ...
    features.append((("vy", action, round(yvelocity)), 1))
    features.append((("vx", action, round(xvelocity)), 1))
...
if __name__ == '__main__':
    ...
    for i in range(num_games):
        ...
        if i % 200 == 0 and i != 0:
            curProb = q.getExProb()
            q.setExplorationProbability(max([curProb - .05, 0]))
